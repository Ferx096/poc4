...
def initialize_client():
    ...
    try:
        # Debug: Listar TODAS las variables que contienen "API" o "AZURE"
        ...
        for key in sorted(os.environ.keys()):
            if "API" in key.upper() or "AZURE" in key.upper():
                ...
                if len(value) > 20:
                    logging.info(
                        f"  {key}: {value[:10]}...{value[-10:]} (len={len(value)})"
                    )
                else:
                    logging.info(f"  {key}: [valor corto oculto]")
        ...
        if api_key:
            ...
            logging.info(f"   Primera parte: {api_key[:20]}...")
            logging.info(f"   Última parte: ...{api_key[-20:]}")
